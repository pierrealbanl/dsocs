...
export default function TableOfContents({ headings }: TableOfContentsProps) {
  ...
  const currentHeadingId = headings.some(
  ...
  const getLinkClassName = (heading: DocumentHeading) =>
    [
      heading.level === 3 ? "table-of-contents__link--nested" : "",
      currentHeadingId === heading.id ? "table-of-contents__link--active" : "",
    ].join(" ");
      ...
      <nav>
        {headings.map((heading) => (
          <button
            ...
            aria-current={
              currentHeadingId === heading.id ? "location" : undefined
            }
          >
            {heading.label}
          </button>
